function define_solar_time_series(solar_dir::String, sys::System)
    ...
    for device in solar_resources
        ...
        # retrive max_active_power
        max_power = get_max_active_power(device) #Q: will Sienna allow a CF value above 1.0?
...
function create_load_fx_timeseries(loads::Vector{PowerLoad}, split_load_ts_dict::Dict{String, Dict{String, DataFrame}})
    ...
    for load in loads
        # Retrieve the name of the current PowerLoad
        load_name = get_name(load)

        #Retrieve max_active_power
        max_power = get_max_active_power(load)
        ...
        if haskey(split_load_ts_dict, load_name)
            ...
            for (year, load_df) in split_load_ts_dict[load_name]
                ...
                normalized_data = load_df[:, :Load] ./ max_power

                # Create the time series by extracting data from load_df
                ts = SingleTimeSeries(
                    name = ts_name,
                    data = TimeArray(load_df[:, :Time], normalized_data)  # Use TimeArray from load_df
                )
