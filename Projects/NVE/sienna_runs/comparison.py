...
@app.cell
def __(pd, sienna_results_folder):
    df_sienna_storage_charge = pd.read_csv(sienna_results_folder / 'storage_charge.csv', index_col=0)
    df_sienna_storage_discharge = pd.read_csv(sienna_results_folder / 'storage_discharge.csv', index_col=0)
    return df_sienna_storage_charge, df_sienna_storage_discharge


@app.cell
def __(df_sienna_dispatch_by_fuel):
    df_sienna_dispatch_by_fuel
    return
...
def __(
    df_sienna_gen,
    df_sienna_storage_charge,
    df_sienna_storage_discharge,
    gen_properties,
    pd,
    sienna_results_folder,
):
    df_sienna_dispatch_by_fuel = df_sienna_gen.T.groupby(gen_properties.fuel_name).sum().T
    df_sienna_dispatch_by_fuel['Storage_Charge'] = -1 *df_sienna_storage_charge.sum(axis=1).values
    df_sienna_dispatch_by_fuel['Storage_Discharge'] = df_sienna_storage_discharge.sum(axis=1).values

    df_sienna_curtailment = pd.read_csv(sienna_results_folder / 'generation_by_fuel.csv').Curtailment
    df_sienna_curtailment.index = df_sienna_curtailment.index # assign timestamp
    df_sienna_dispatch_by_fuel['curtailment'] = df_sienna_curtailment
    df_sienna_dispatch_by_fuel.curtailment = df_sienna_dispatch_by_fuel.curtailment.clip(lower=0) # clip small negative curtailment values

    ### modify plot to show imports as thermal
    # df_sienna_dispatch_by_fuel['Natural gas'] = df_sienna_dispatch_by_fuel['Natural gas'] + df_sienna_gen[['Southern Purchases (NVP)','Northern Purchases (Sierra)']].sum(axis=1)
    # df_sienna_dispatch_by_fuel['PV'] = df_sienna_dispatch_by_fuel['PV'] - df_sienna_gen[['Southern Purchases (NVP)','Northern Purchases (Sierra)']].sum(axis=1)
    # # df_sienna_fuel
    return df_sienna_curtailment, df_sienna_dispatch_by_fuel
...
def __(df_plexos_gen_by_fuel, df_sienna_dispatch_by_fuel):
    ...
    sienna = df_sienna_dispatch_by_fuel
...
def __(pd, plexos, sienna):
    ...
    end_time = pd.Timestamp(2030, 1, 3)
...
@app.cell
def __(colors_fuel, plexos, plt, sienna):
...
@app.cell
def __(df_plexos_generation, df_sienna_gen):
    df_plexos_generation.loc[:, ~df_plexos_generation.columns.isin(df_sienna_gen.columns)]
    return
